...
describe("/api/vulnerabilities", () => {
  ...
  it("should return vulnerabilities data with no parameters (cache miss)", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "http://localhost:8000/vulnerabilities/flatten",
    );
  });
  ...
  it("should handle cluster and namespace parameters", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "http://localhost:8000/vulnerabilities/flatten?cluster=test-cluster&namespace=test-namespace",
    );
  });
  ...
  it("should use configured backend URL", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "http://custom-backend:9000/vulnerabilities/flatten",
    );
  });
  ...
  it("should properly encode URL parameters", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "http://localhost:8000/vulnerabilities/flatten?cluster=test-cluster&namespace=test%2Fnamespace",
    );
  });
});
